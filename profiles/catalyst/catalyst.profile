 ...
 */
function catalyst_profile_modules() {
  $modules = array(
 ...
 */
function catalyst_profile_details() {
  return array(
    'name' => 'Catalyst Drupal',
    'description' => 'Acquia enchanced Drupal with a Catalyst developer profile.'
  );
}
 ...
 *   task list.
 */
function catalyst_profile_task_list() {
  ...
  $conf['theme_settings'] = array(
    'default_logo' => 0,
    'logo_path' => 'profiles/catalyst/CatalystLogo.png');
 ...
 */
function catalyst_profile_tasks(&$task, $url) {
  global $language;
  ...
  $welcome_file = 'profiles/catalyst/' . $language->language . '/welcome.txt';
  if (!file_exists($welcome_file)) {
    drupal_set_message(t('Could not find file !filepath', array('!filepath' => $welcome_file)), 'error');
    $welcome_file = 'profiles/catalyst/en/welcome.txt';
  }
  if ($welcome_node = _catalyst_profile_create_node($welcome_file, 'page')) {
    node_save($welcome_node);
    variable_set('catalyst_welcome', $welcome_node->nid);
  } else {
  ...
  variable_set('web_test_case_profile', 'catalyst');
 ...
 */
function catalyst_form_alter(&$form, $form_state, $form_id) {
 ...
 */
function _catalyst_profile_create_node($filename, $page_type) {
